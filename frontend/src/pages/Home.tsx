...
function PageHeading({ count }: { count: number | null }) {
  return (
    <div className="flex items-end justify-between gap-4 border-b border-hairline pb-4">
      <div>
        <h1 className="serif text-5xl leading-none text-ink">
          Packages
        </h1>
        {count !== null && (
...
function ColumnHeader() {
  return (
    <div className="-mx-4 grid grid-cols-[minmax(0,1.4fr)_minmax(100px,1.3fr)_5rem_4.5rem] gap-4 px-4 pb-2 text-[10px] uppercase tracking-micro text-ink-faint sm:grid-cols-[minmax(0,1.4fr)_minmax(120px,1.4fr)_5rem_4.5rem_4.5rem]">
      <span>Name</span>
      <span>Usage</span>
      <span>Status</span>
      <span className="hidden text-right sm:inline">Created</span>
    ...
    <div aria-busy="true">
      ...
      <ul className="border-y border-hairline divide-y divide-hairline">
        {[0, 1, 2, 3].map((i) => (
          <li
            key={i}
            className="-mx-4 grid grid-cols-[minmax(0,1.4fr)_minmax(100px,1.3fr)_5rem_4.5rem] items-center gap-4 px-4 py-4 sm:grid-cols-[minmax(0,1.4fr)_minmax(120px,1.4fr)_5rem_4.5rem_4.5rem]"
          >
            <div className="h-4 w-32 animate-pulse rounded-sm bg-ink/5" />
            <div className="h-1.5 animate-pulse rounded-sm bg-ink/5" />
            <div className="h-3 w-12 animate-pulse rounded-sm bg-ink/5" />
            <div className="hidden h-3 animate-pulse rounded-sm bg-ink/5 sm:block" />
...
function Empty() {
  return (
    <div className="pt-6 text-center">
      <p className="serif text-3xl italic text-ink">No packages yet.</p>
